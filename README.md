...
`0.1.0-rc.19` is a metadata-only coordination release for PowerMeter V2 server `v0.1.0-rc.19`. Its stateless runtime is byte-identical to public RC18: it uses `pm-telemetry/2.0.0`, preserves `pm-protocol/1.0.0` for control/authentication/OTA, and binds the generated server OpenAPI SHA-256 `1f0fe0aed5fe187a6c22523469dc9d2e76de9f5c75bed4433a654e339968deda`. RC17 remains immutable failed-candidate evidence, and RC18 remains immutable public runtime evidence. The active build excludes SD, backlog, missing-prefix, and contiguous-acknowledgement components. `CONFIG_PM_HARDWARE_IDENTITY_VERIFIED` remains disabled, release artifacts remain `hardware_certification: pending`, and stable promotion still requires exact marked-unit evidence, electrical validation, and the 72-hour suite; see [hardware identity](docs/HARDWARE_IDENTITY.md) and [hardware certification](docs/HARDWARE_CERTIFICATION.md).

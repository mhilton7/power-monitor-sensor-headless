...
`0.1.0-rc.15` repairs backlog delivery without changing the server contract or measurement path. Oversized reading batches are deterministically reduced to the largest complete JSON body that fits the authenticated 8,192-byte transport limit; unsent records remain on microSD. A complete all-segment inventory can now prove and report a permanent missing prefix, while incomplete or unreadable inventories fail closed. Server RC14 remains compatible, `pm-protocol/1.0.0` is unchanged, and public RC14 remains immutable. `CONFIG_PM_HARDWARE_IDENTITY_VERIFIED` remains disabled, release artifacts remain `hardware_certification: pending`, and stable promotion still requires exact marked-unit evidence, electrical validation, and the 72-hour suite; see [hardware identity](docs/HARDWARE_IDENTITY.md) and [hardware certification](docs/HARDWARE_CERTIFICATION.md).

...
`0.1.0-rc.20` is a metadata-only coordination release for PowerMeter V2 server `v0.1.0-rc.20`. Its stateless runtime sources and profiles are unchanged from public RC19: it uses `pm-telemetry/2.0.0`, preserves `pm-protocol/1.0.0` for control/authentication/OTA, and binds the generated server OpenAPI SHA-256 `dd49a242ebcd374b3e7574742aa010a29d4c5dab2008960ea9e0c842c62fd915`. RC17 remains immutable failed-candidate evidence, and RC18/RC19 remain immutable public runtime evidence. The active build excludes SD, backlog, missing-prefix, and contiguous-acknowledgement components. `CONFIG_PM_HARDWARE_IDENTITY_VERIFIED` remains disabled, release artifacts remain `hardware_certification: pending`, and stable promotion still requires exact marked-unit evidence, electrical validation, and the 72-hour suite; see [hardware identity](docs/HARDWARE_IDENTITY.md) and [hardware certification](docs/HARDWARE_CERTIFICATION.md).

...
`0.1.0-rc.12` advances only the immutable release identity and generated server-contract binding required by the coordinated server RC12 publication. Runtime behavior is unchanged from public RC11: the supported FAT-volume capacity API refreshes mounted-card capacity periodically, trusted-time synchronization retries while time is untrusted, response-header authentication remains corrected, and PZEM polling remains strict and read-only. `CONFIG_PM_HARDWARE_IDENTITY_VERIFIED` remains disabled, release artifacts remain `hardware_certification: pending`, and stable promotion still requires exact marked-unit evidence, electrical validation, and the 72-hour suite. Public RC11 remains immutable. `pm-protocol/1.0.0` is unchanged; see [hardware identity](docs/HARDWARE_IDENTITY.md) and [hardware certification](docs/HARDWARE_CERTIFICATION.md).

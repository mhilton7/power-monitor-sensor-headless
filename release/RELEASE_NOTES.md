# PowerMeter Sensor Headless 0.1.0-rc.24

RC24 is the metadata-only coordinated firmware candidate for corrected PowerMeter V2 server `v0.1.0-rc.24` (deterministic firmware build 27). Runtime sources, partition layout, sdkconfig profiles, and dependency locks remain byte-identical to public firmware RC23. The image is rebuilt under the new immutable semantic version, so its embedded ESP-IDF application descriptor, ELF build identifier, and image digest must be independently verified and must not reuse RC23 values. It retains `pm-telemetry/2.0.0` plus authenticated control, enrollment, provisioning, and OTA under `pm-protocol/1.0.0`. The generated server OpenAPI document is bound by the exact SHA-256 recorded in `test/vectors/server-contract.json` and the release compatibility asset.
...
RC24 remains a release candidate: `CONFIG_PM_HARDWARE_IDENTITY_VERIFIED=n`, hardware certification is pending, and no physical sensor migration or OTA success is claimed by this release build. Stable promotion still requires exact marked-unit electrical, rollback, recovery, and soak evidence.

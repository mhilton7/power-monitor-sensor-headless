# PowerMeter Sensor Headless 0.1.0-rc.2

This release candidate carries forward the ESP32-S3 headless measurement agent, authenticated outbound protocol, append-only microSD outage journal, USB/COM provisioning and repair, durable commands, and dual-slot OTA rollback. RC2 updates only release identity and compatible-server metadata so it can pair immutably with the server's first-sensor enrollment fix; firmware protocol and runtime behavior remain unchanged from RC1.

Its exact enrollment, heartbeat, reading-batch, permanent-loss, OTA-download, destructive-command, credential-rotation, HKDF, and base64 HMAC contracts are locked by cross-repository vectors and validated against the compatible PowerMeter V2 server `v0.1.0-rc.2`. Firmware also authenticates server response bytes, timestamps, and nonces before applying acknowledgements or commands.

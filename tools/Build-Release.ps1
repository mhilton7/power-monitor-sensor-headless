[CmdletBinding()]
param(
    [ValidatePattern('^[0-9]+\.[0-9]+\.[0-9]+-rc\.[0-9]+$')][string]$Version = '0.1.0-rc.13',
    [ValidateRange(1,2147483647)][int]$BuildNumber = 1,
    [ValidatePattern('^https://')][string]$DownloadBase = 'https://power-monitor.home.arpa:8443/api/firmware/releases/0.1.0-rc.13',
    [string]$BuildDirectory = (Join-Path $PSScriptRoot '..\build-release'),
    [string]$OutputDirectory = (Join-Path $PSScriptRoot '..\release\out\0.1.0-rc.13')
)

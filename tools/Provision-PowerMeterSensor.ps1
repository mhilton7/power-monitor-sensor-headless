...
param(
    ...
    [Parameter(Mandatory)][AllowEmptyString()][string]$ServerOrigin,
    [Parameter(Mandatory)][AllowEmptyString()][string]$CaCertificatePath,
...
$canonicalServerOrigin = ConvertTo-PowerMeterServerOrigin -Value $ServerOrigin
$caPem = Read-PowerMeterCaCertificate -Path $CaCertificatePath
...
try {
    ...
    $config = [ordered]@{
        ...
        server_origin = $canonicalServerOrigin
        ca_pem = $caPem
    ...
    $transaction = Invoke-PowerMeterProvisioningTransaction -Serial $serial -Config $config `
        -PortName $selectedPort
    Write-Host "Configuration tests: $($transaction.Tested.stage)"
    Write-Host "Configuration $($transaction.Committed.stage); secrets were not returned by the device."
} finally {
    $wifiPlain = $null
    $tokenPlain = $null
